    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (security != null) {
                    handler.setSecurity(security.value());
                }
...
//        method.invoke() //
        ...
        //判断是否有权限
        if (handler.getSecurity() != null && !handler.getSecurity().equals(req.getParameter("username"))) {
            resp.setContentType("text/json;charset=UTF-8");
            resp.setCharacterEncoding("UTF-8");
            PrintWriter out = resp.getWriter();
            String str = req.getParameter("username") + "没有权限";
            out.println(str);
            out.flush();
            out.close();
        ...
        try {
            Object obj = handler.getMethod().invoke(handler.getController(), paraValues);
